...
func Example_rollupTags() {
	...
	store, err := tsdb.Open(ctx, tsdb.SQLite(
	...
	series, err := store.Series(ctx, tsdb.AggregateQuery{
		Query: tsdb.Query{
			MetricName: "gpu.usage",
			EntityID:   "host-1",
			Start:      base,
...
func Example_agentFleetLifecycle() {
	...
	for _, def := range []tsdb.Definition{
		{Name: "cpu.usage", Type: tsdb.TypeGauge, Unit: "%", RetentionDays: 30},
		{Name: "mem.usage", Type: tsdb.TypeGauge, Unit: "%", RetentionDays: 30},
		{Name: "disk.usage", Type: tsdb.TypeGauge, Unit: "%", RetentionDays: 30},
		{Name: "swap.usage", Type: tsdb.TypeGauge, Unit: "%", RetentionDays: 30},
		{Name: "net.in.bytes", Type: tsdb.TypeCounter, Unit: "bytes", RetentionDays: 30},
		{Name: "net.out.bytes", Type: tsdb.TypeCounter, Unit: "bytes", RetentionDays: 30},
		{Name: "process.count", Type: tsdb.TypeGauge, Unit: "count", RetentionDays: 30},
		{Name: "gpu.usage", Type: tsdb.TypeGauge, Unit: "%", RetentionDays: 30},
		{Name: "ping.latency_ms", Type: tsdb.TypeGauge, Unit: "ms", RetentionDays: 30},
	} {
	...
	for sec := 0; sec < 120; sec++ {
		...
		case 45:
			...
			if _, err := store.DeleteSeries(ctx, tsdb.Query{
		...
		case 70:
			...
			if err := store.UpsertMetric(ctx, tsdb.Definition{
				Name: "ping.latency_ms", Type: tsdb.TypeGauge, Unit: "ms",
				RetentionDays: 30}); err != nil {
				log.Fatal(err)
			}
		case 85:
		...
		points := make([]tsdb.Point, 0, len(agents)*(7+2+len(pingTasks)))
	...
	activeCPU, err := store.Series(ctx, tsdb.AggregateQuery{
		Query: tsdb.Query{
	...
	deletedAgentCPU, err := store.Series(ctx, tsdb.AggregateQuery{
		Query: tsdb.Query{
			MetricName: "cpu.usage",
			EntityID:   "agent-007",
			Start:      base,
	...
	task1, err := store.Series(ctx, tsdb.AggregateQuery{
		Query: tsdb.Query{
			MetricName: "ping.latency_ms",
			Start:      base,
			End:        base.Add(2 * time.Minute),
	...
	task2, err := store.Series(ctx, tsdb.AggregateQuery{
		Query: tsdb.Query{
			MetricName: "ping.latency_ms",
			Start:      base,
			End:        base.Add(2 * time.Minute),
...
func systemPoints(agentID string, ts time.Time, sec int) []tsdb.Point {
...
func nonEmpty(points []tsdb.AggregatePoint) []tsdb.AggregatePoint {
	out := points[:0]
