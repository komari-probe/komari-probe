...
// OnConnect implements wsconn.FrameInterceptor. It runs the wsConnect
// hooks registered for the connection path; the first denial rejects the
// wsconn.
func (m *Manager) OnConnect(info *wsconn.ConnInfo) (deny bool, reason string) {
...
// OnMessage implements wsconn.FrameInterceptor for inbound frames.
func (m *Manager) OnMessage(info *wsconn.ConnInfo, frameType int, data []byte) (int, []byte, bool) {
	return m.runFrameHooks(hookWSMessage, info, frameType, data)
}

// OnSend implements wsconn.FrameInterceptor for outbound frames.
func (m *Manager) OnSend(info *wsconn.ConnInfo, frameType int, data []byte) (int, []byte, bool) {
...
// frame through.
func (m *Manager) runFrameHooks(kind hookKind, info *wsconn.ConnInfo, frameType int, data []byte) (int, []byte, bool) {
...
// OnClose implements wsconn.FrameInterceptor for connection teardown.
func (m *Manager) OnClose(info *wsconn.ConnInfo) {
...
func wsHookContext(vm *goja.Runtime, info *wsconn.ConnInfo) *goja.Object {
	ctx := vm.NewObject()
...
func wsFrameObject(vm *goja.Runtime, info *wsconn.ConnInfo, frameType int, data []byte) *goja.Object {
	msg := vm.NewObject()
...
var _ wsconn.FrameInterceptor = (*Manager)(nil)
