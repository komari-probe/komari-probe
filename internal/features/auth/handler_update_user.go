...
func UpdateUser(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		respond.Error(c, 400, "Invalid or missing request body: "+err.Error())
		return
	}
	if req.Password == nil && req.Name == nil {
		respond.Error(c, 400, "At least one field (username or password) must be provided")
		return
	}
	if req.Name != nil && len(*req.Name) < 3 {
		respond.Error(c, 400, "Username must be at least 3 characters long")
		return
	}
	if req.Password != nil && len(*req.Password) < 6 {
		respond.Error(c, 400, "Password must be at least 6 characters long")
		return
	}
	if req.Password != nil {
		c.Set("2fa_code", req.TwoFa)
		if err := VerifySensitive2FA(c); err != nil {
			respond.Error(c, 401, err.Error())
			return
		}
	}
	if err := updateUserRecord(req.Uuid, req.Name, req.Password, req.SsoType); err != nil {
		respond.Error(c, 500, "Failed to update user: "+err.Error())
		return
	}
	uuid, _ := c.Get("uuid")
	auditlog.Log(c.ClientIP(), uuid.(string), "User updated", "warn")
	respond.Success(c, gin.H{"uuid": req.Uuid})
}
