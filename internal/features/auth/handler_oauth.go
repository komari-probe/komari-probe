...
func OAuth(c *gin.Context) {
	...
	authURL, state := oauth.CurrentProvider().GetAuthorizationURL(GetCallbackURL(c))
...
func OAuthCallback(c *gin.Context) {
	...
	oidcUser, err := oauth.CurrentProvider().OnCallback(c, state, queries, GetCallbackURL(c))
	if err != nil {
	...
	// 创建会话
	session, err := CreateSession(user.UUID, sessionCookieMaxAge, c.Request.UserAgent(), c.ClientIP(), "oauth")
	if err != nil {
