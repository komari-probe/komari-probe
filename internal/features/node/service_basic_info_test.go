...
func (p staticGeoIPProvider) GetGeoInfo(ip net.IP) (*geoipprovider.GeoInfo, error) {
	return &geoipprovider.GeoInfo{ISOCode: p.iso, Name: p.iso}, nil
}
...
func TestV2BasicInfoFillsRegionFromGeoIP(t *testing.T) {
	...
	want := geoipprovider.GetRegionUnicodeEmoji("SG")
	if got.Region != want {
