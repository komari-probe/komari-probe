	...
	go func() {
		once.Do(func() {
			...
			for _, provider := range all {
				if _, err := GetConfigByName(provider.GetName()); err == nil {
					continue
				}
				...
				if err := SaveConfig(&models.MessageSenderProvider{
...
func SendEvent(event models.EventMessage) error {
	...
	if !cfg[settings.NotificationEnabledKey].(bool) {
		return nil
	}
	...
	// 如果没有实现,使用模板格式化为文本消息
	messageTemplate := cfg[settings.NotificationTemplateKey].(string)
