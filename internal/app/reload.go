...
type reloadHandler struct {
	name    string
	handler func(kv.ConfigEvent)
}

// ReloadManager 统一收敛所有配置热重载逻辑。
//
// 过去 kv.Subscribe 分散在 cmd/server.go、cors.go 等多处，导致：
//   - 谁监听哪些 key 不清晰；
//   - 某个 handler panic 会连带影响同一订阅回调里的其它逻辑；
//   - 关闭时无法统一停止。
//
// ReloadManager 把这些 handler 注册在一处，用单个 kv.Subscribe 分发，
...
func (m *ReloadManager) Register(name string, handler func(kv.ConfigEvent)) {
	if handler == nil {
		return
	}
...
func (m *ReloadManager) Start() {
	...
	m.started = true
	kv.Subscribe(func(event kv.ConfigEvent) {
...
func (m *ReloadManager) dispatch(h reloadHandler, event kv.ConfigEvent) {
	defer func() {
		if r := recover(); r != nil {
