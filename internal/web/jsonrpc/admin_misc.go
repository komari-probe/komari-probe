...
func adminEditSettings(ctx context.Context, req *rpc.JsonRpcRequest) (any, *rpc.JsonRpcError) {
	...
	if err := metricstore.ValidateRollupSettingChanges(cfg); err != nil {
	...
	if metricstore.ConfigKeysTouched(cfg) {
		merged, err := metricstore.NormalizeAndMergeConfig(cfg)
		if err != nil {
			return nil, rpc.MakeError(rpc.InternalError, "Failed to resolve metric store config: "+err.Error(), nil)
		}
		testCtx, cancel := context.WithTimeout(ctx, 15*time.Second)
		err = metricstore.TestConnection(testCtx, merged)
		cancel()
		if err != nil {
	...
	outcome, err := metricstore.ReloadAfterConfigChange(ctx, cfg)
	if err != nil {
