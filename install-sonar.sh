...
DATA_DIR="/opt/sonar"
SERVICE_NAME="sonar"
BINARY_PATH="$INSTALL_DIR/sonar"
BACKUP_DIR="$INSTALL_DIR/backup"
DATA_BACKUP_DIR="$DATA_DIR/data/backup"
DEFAULT_PORT="25774"
LISTEN_PORT=""
STANDARD_REPO="sonar-probe/sonar"

# Backward compatibility / legacy detection
if [ ! -f "$BINARY_PATH" ] && [ -f "/opt/komari/komari" ]; then
    INSTALL_DIR="/opt/komari"
    DATA_DIR="/opt/komari"
    SERVICE_NAME="komari"
    BINARY_PATH="/opt/komari/komari"
    BACKUP_DIR="$INSTALL_DIR/backup"
    DATA_BACKUP_DIR="$DATA_DIR/data/backup"
elif [ ! -f "$BINARY_PATH" ] && [ -f "/opt/komari/sonar" ]; then
    INSTALL_DIR="/opt/komari"
    DATA_DIR="/opt/komari"
    SERVICE_NAME="komari"
    BINARY_PATH="/opt/komari/sonar"
    BACKUP_DIR="$INSTALL_DIR/backup"
    DATA_BACKUP_DIR="$DATA_DIR/data/backup"
fi
REPO="$STANDARD_REPO"
...
msg() {
    ...
    case "$key" in
        ...
        port_title)
            ...
            ;;
        port_prompt)
            en_text='Enter the Sonar listen port (1-65535)'
            zh_text='请输入 Sonar 的监听端口（1-65535）'
            ;;
        invalid_port)
        ...
        no_systemd_manual)
            ...
            ;;
        service_started)
            en_text='Sonar service started successfully.'
            zh_text='Sonar 服务启动成功。'
            ;;
        service_start_failed)
            en_text='Sonar service failed to start.\n\nView logs: journalctl -u %s -f'
            zh_text='Sonar 服务启动失败。\n\n查看日志：journalctl -u %s -f'
            ;;
        systemd_start)
        ...
        access_info)
            ...
            ;;
        cleanup_confirm)
            en_text='This will delete Sonar binary upgrade backups and data upgrade archives.\n\nContinue?'
            zh_text='将删除 Sonar 的二进制升级备份和数据升级压缩包。\n\n确定继续吗？'
            ;;
        cleanup_cancelled)
        ...
        systemd_required)
            ...
            ;;
        stopping_service)
            en_text='Stopping Sonar service...'
            zh_text='停止 Sonar 服务...'
            ;;
        clearing_backups)
        ...
        upgrade_start_failed_restore_failed)
            ...
            ;;
        uninstall_start)
            en_text='Uninstalling Sonar...'
            zh_text='卸载 Sonar...'
            ;;
        confirm_uninstall)
            en_text='This will remove the Sonar binary and service.\n\nContinue?'
            zh_text='这将删除 Sonar 二进制文件和服务。\n\n确定继续吗？'
            ;;
        uninstall_cancelled)
        ...
        data_dir_not_empty)
            ...
            ;;
        binary_removed)
            en_text='Sonar binary removed.'
            zh_text='Sonar 二进制文件已删除。'
            ;;
        uninstall_complete)
            ...
            ;;
        service_status)
            en_text='Sonar service status:'
            zh_text='Sonar 服务状态：'
            ;;
        service_logs)
            en_text='View Sonar service logs (press Ctrl+C to exit)...'
            zh_text='查看 Sonar 服务日志（按 Ctrl+C 退出）...'
            ;;
        restart_start)
            en_text='Restarting Sonar service...'
            zh_text='重启 Sonar 服务...'
            ;;
        restart_success)
        ...
        main_prompt)
            ...
            ;;
        main_install)
            en_text='Install Sonar'
            zh_text='安装 Sonar'
            ;;
        main_upgrade)
            en_text='Upgrade Sonar'
            zh_text='升级 Sonar'
            ;;
        main_uninstall)
            en_text='Uninstall Sonar'
            zh_text='卸载 Sonar'
            ;;
        main_status)
...
# Check if Sonar is already installed
is_installed() {
...
[Unit]
Description=Sonar Monitor Service
